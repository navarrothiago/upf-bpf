...
TEST_F(SessionManagerTests, managePDR)
{
  ...
  EXPECT_TRUE(mpSessionManager->lookupPDR(pSession->seid, pPdrUpdated->pdr_id)->far_id.far_id != pPdr->far_id.far_id);
  EXPECT_NO_THROW(mpSessionManager->removePDR(pSession->seid, pPdrUpdated));
